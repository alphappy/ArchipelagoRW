...
# Update this when game version updates
gameversion = "1.11.7b"
scugs_by_dlcstate = {
    "Vanilla": scugs_vanilla, "MSC": scugs_msc, "Watcher": scugs_watcher, "MSC_Watcher": scugs_msc_watcher
}

########################################################################################################################

for dlcstate, all_scugs in scugs_by_dlcstate.items():
    dlcstate_data = data.setdefault(dlcstate, {})
    ...
    for fp in glob(path.join(ROOT_FP, gameversion, dlcstate, 'world', '*', 'world_*.txt')):
        ...
        region_data = dlcstate_data.setdefault(region, {})
...
####################################################################################################################
    ...
    for fp in (sorted(list(normal_settings_files)) + sorted(list(all_settings_files - normal_settings_files))):
        ...
        try:
            room_data = dlcstate_data[region][room]
        except KeyError:
...
if args.create_debug_file:
    with open("debug_data.txt", "w") as f1:
        pprint.pp(data, f1, compact=True)
